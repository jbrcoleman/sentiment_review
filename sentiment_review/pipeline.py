...
def ingest(file_link=None):
    ...
    with open(f"data/yelp_review-{DATE}.json", "wb") as file_obj:
        storage_client.download_blob_to_file(file_link, file_obj)
...
def transform_json_to_csv():
    ...
    for chunk_review in review:
        ...
        chunk_review = chunk_review.drop(
            ["review_id", "user_id", "business_id", "date", "useful", "funny", "cool"],
            axis=1,
        )
    ...
    reviewdf["text"] = reviewdf["text"].apply(
        lambda x: text_clean(x)  # pylint: disable=W0108
    )
    reviewdf["text"] = reviewdf["text"].apply(
        lambda x: remove_punc(x)  # pylint: disable=W0108
    )
    stop_words = set(stopwords.words("english"))
    reviewdf["text"] = reviewdf["text"].apply(
        lambda x: remove_stopwords(x, stop_words)  # pylint: disable=W0108
    )
...
def upload():
    ...
    # Name of the object in local file system # pylint: disable=W0108
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pass a yelp review json file to be uploaded to big query"
    )
